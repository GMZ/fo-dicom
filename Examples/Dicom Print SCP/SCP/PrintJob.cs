...
namespace Dicom.PrintScp
{
    public class PrintJob : DicomDataset
    {
        ...
        // not use in this demo but would be used in a production version
        ...
        public FilmSession Session { get; private set; }

        public List<FilmBox> FilmBoxList { get; private set; }
        ...
        private void PreviewProc()
        {
            ...
            var ts = _previewDialog.Controls.OfType<ToolStrip>().First(ctl => ctl.Name.Equals("toolStrip1"));
            if (ts != null)
            {
                var printButton = ts.Items.OfType<ToolStripItem>().First(tsi => tsi.Name.Equals("printToolStripButton")) as ToolStripButton;
                if (printButton != null)
                {
                    ts.Items.Remove(printButton);
                    var b = new ToolStripButton
                    {
                        ImageIndex = printButton.ImageIndex, Visible = true
                    };
                    ts.Items.Insert(0, b);
        ...
        {
            var printDialog1 = new PrintDialog { Document = _printDocument };
